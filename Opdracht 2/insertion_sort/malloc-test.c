...
#define MALLOC_LOOP(res, init, err)                                         \
    do {                                                                       \
        res = init;                                                            \
        size_t old_non_fail_amount = malloc_non_fail_amount,                   \
               old_free_count = free_count;                                    \
        if (err == res) {                                                     \
...
START_TEST(test_init_no_malloc)
{
    ...
    if (list_add_front(l, list_new_node(2))) ck_assert(0);
    n = list_head(l);
    if (list_unlink_node(l, n)) ck_assert(0);
    if (list_node_value(n) != 2) ck_assert(0);
    list_free_node(n);
    list_cleanup(l);
